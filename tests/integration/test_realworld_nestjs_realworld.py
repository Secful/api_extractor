"""Integration tests for NestJS extractor with nestjs-realworld-example-app."""
...
def realworld_path():
    ...
    if not os.path.exists(path):
        pytest.skip("NestJS RealWorld fixture not available (run: git submodule update --init)")
    return path


class TestNestJSRealWorld:
    """Integration tests for nestjs-realworld-example-app."""

    def test_extraction(self, realworld_path: str) -> None:
        """Test extraction from nestjs-realworld-example-app.
        ...
        assert len(result.endpoints) == 21, \
            f"Expected exactly 21 endpoints, found {len(result.endpoints)}"
    ...
    def test_article_endpoints(self, realworld_path: str) -> None:
        ...
        assert "/articles/{slug}/comments" in paths, "Should find comments endpoint"

    def test_user_endpoints(self, realworld_path: str) -> None:
        """Test that user authentication and management endpoints are extracted."""
        extractor = NestJSExtractor()
        result = extractor.extract(realworld_path)
    ...
    def test_slug_based_routing(self, realworld_path: str) -> None:
        ...
        assert len(slug_endpoints) == 9, \
            f"Expected exactly 9 endpoints with {{slug}} parameter, found {len(slug_endpoints)}"
    ...
    def test_path_parameters(self, realworld_path: str) -> None:
        ...
        assert len(param_endpoints) == 12, \
            f"Expected exactly 12 endpoints with path parameters, found {len(param_endpoints)}"
        ...
        paths = {ep.path for ep in result.endpoints}
        assert any("{slug}" in p for p in paths), "Should find {slug} parameter"
        assert any("{username}" in p for p in paths), "Should find {username} parameter"
    ...
    def test_unique_paths(self, realworld_path: str) -> None:
        ...
        assert len(unique_paths) == 14, \
            f"Expected exactly 14 unique paths, found {len(unique_paths)}"

    def test_root_endpoint(self, realworld_path: str) -> None:
